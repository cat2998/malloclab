...
static void *coalesce(void *str)
{
    size_t prev_alloc = GET_ALLOC(FTRP(PREV_BLKP(str)));
    size_t next_alloc = GET_ALLOC(HDRP(NEXT_BLKP(str)));
    size_t size = GET_SIZE(HDRP(str));
    ...
    { /* Case 2 */
        size += GET_SIZE(HDRP(NEXT_BLKP(str)));
        PUT(HDRP(str), PACK(size, 0));
        PUT(FTRP(str), PACK(size, 0));
    }
    else if (!prev_alloc && next_alloc)
    { /* Case 3 */
        size += GET_SIZE(HDRP(PREV_BLKP(str)));
        PUT(FTRP(str), PACK(size, 0));
        PUT(HDRP(PREV_BLKP(str)), PACK(size, 0));
        str = PREV_BLKP(str);
    }
    else
    { /* Case 4 */
        size += GET_SIZE(HDRP(PREV_BLKP(str))) +
                GET_SIZE(FTRP(NEXT_BLKP(str)));
        PUT(HDRP(PREV_BLKP(str)), PACK(size, 0));
        PUT(FTRP(NEXT_BLKP(str)), PACK(size, 0));
        str = PREV_BLKP(str);
    }
    return str;
}
